    ...
    def validate(self, attrs):
        ...
        try:
            invitation = EmployeeInvitation.objects.get(
                email=email, otp=otp, accepted_at__isnull=True, rejected_at__isnull=True
            )
...
class EmployeeInvitationResponseSerializer(serializers.ModelSerializer):
    ...
    phoneNumber = serializers.CharField(
        source="employeephone", required=False, allow_blank=True
    )
    department = serializers.CharField(
        source="employeedepartment", required=False, allow_blank=True
    )
            ...
            try:
                ...
                if check_password(password, organization.password):
                    if organization.owner and organization.owner.is_active:
                        ...
                        raise serializers.ValidationError(
                            "Organization account is not properly configured."
                        )
                else:
...
class EmployeeOnboardingSerializer(serializers.Serializer):
    ...
    def update(self, user, validated_data):
        ...
        PSS10Assessment.objects.create(user=user, score=pss10_total, category=category)
...
class PasswordResetOTPVerificationSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        otp = (
            PasswordResetToken.objects.filter(code=code).order_by("-created_at").first()
        )
        if not otp:
...
class MFAVerifySerializer(serializers.Serializer):
    temp_token = serializers.CharField(
        required=True, help_text="Temporary token received after login step 1"
    )
    code = serializers.CharField(
        max_length=6,
        required=True,
    ...
    def validate(self, data):
        ...
        if assessment_type == "GAD-7":
            if not gad7_responses:
                raise serializers.ValidationError(
                    "GAD-7 responses are required for GAD-7 assessment."
                )
            if len(gad7_responses) != 7:
                raise serializers.ValidationError(
                    "GAD-7 must have exactly 7 responses."
                )

        elif assessment_type == "PHQ-9":
            if not phq9_responses:
                raise serializers.ValidationError(
                    "PHQ-9 responses are required for PHQ-9 assessment."
                )
            if len(phq9_responses) != 9:
                raise serializers.ValidationError(
                    "PHQ-9 must have exactly 9 responses."
                )

        elif assessment_type == "BOTH":
            if not gad7_responses or not phq9_responses:
                raise serializers.ValidationError(
                    "Both GAD-7 and PHQ-9 responses are required."
                )
            if len(gad7_responses) != 7:
                raise serializers.ValidationError(
                    "GAD-7 must have exactly 7 responses."
...
class MentalHealthAssessmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if assessment_type == "GAD-7":
            if not gad7_scores:
                raise serializers.ValidationError(
                    "GAD-7 scores are required for GAD-7 assessment."
                )
            ...
            for score in gad7_scores:
                if not isinstance(score, int) or score < 0 or score > 3:
                    raise serializers.ValidationError(
                        "Each GAD-7 score must be between 0 and 3."
                    )

        elif assessment_type == "PHQ-9":
            if not phq9_scores:
                raise serializers.ValidationError(
                    "PHQ-9 scores are required for PHQ-9 assessment."
                )
            ...
            for score in phq9_scores:
                if not isinstance(score, int) or score < 0 or score > 3:
                    raise serializers.ValidationError(
                        "Each PHQ-9 score must be between 0 and 3."
                    )

        elif assessment_type == "BOTH":
            if not gad7_scores or not phq9_scores:
                raise serializers.ValidationError(
                    "Both GAD-7 and PHQ-9 scores are required."
                )
            if len(gad7_scores) != 7:
                raise serializers.ValidationError("GAD-7 must have exactly 7 scores.")
            if len(phq9_scores) != 9:
            ...
            for score in gad7_scores + phq9_scores:
                if not isinstance(score, int) or score < 0 or score > 3:
                    raise serializers.ValidationError(
                        "Each score must be between 0 and 3."
                    )
...
class SubscriptionInitiateSerializer(serializers.Serializer):
    ...
    def validate_plan_id(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError(
                _(
                    "The selected subscription plan is invalid or not currently available."
                )
            )
...
class ClientEngagementSerializer(serializers.ModelSerializer):
    ...
    engagement_display = serializers.CharField(
...
class OrganizationsManagementSerializer(serializers.ModelSerializer):
    ...
    status_display = serializers.CharField(
        source="get_is_active_display", read_only=True
    )
    ...
    def get_is_saved(self, obj) -> bool:
        ...
        if request and request.user.is_authenticated:
            return SavedResource.objects.filter(
                user=request.user, meditation=obj
            ).exists()
        return False
...
class AssessmentResponseSerializer(serializers.ModelSerializer):
    ...
    def validate_responses(self, value):
        ...
        for score in value:
            if not isinstance(score, int) or score < 0 or score > 3:
                raise serializers.ValidationError(
                    "Each response must be a score between 0 and 3."
                )
...
class AdminUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if role == "employer":
            org = attrs.get("organization") or getattr(
                self.instance, "organization", None
            )
            if not org:
                raise serializers.ValidationError(
                    "Employer must belong to an organization."
                )
        ...
        if (
            role == "employee"
            and not avatar
            and not getattr(self.instance, "avatar", None)
        ):
            ...
            if attrs.get("is_active") is True:
                raise serializers.ValidationError(
                    "Employee must have an avatar before activation."
                )
        return attrs
...
class MediaSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        media_type = (
            data.get("media_type") or self.instance.media_type
        ...
        if media_type == Media.ARTICLE:
            body = (
                data.get("body")
                if "body" in data
                else getattr(self.instance, "body", "")
            )
            if not body:
            ...
            file = (
                data.get("file")
                if "file" in data
                else getattr(self.instance, "file", None)
            )
            if not file:
                raise serializers.ValidationError(
                    "Audio and Video items must include a file upload."
                )
